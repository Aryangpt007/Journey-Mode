...
public class JourneyDataAttachment {
     ...
     * Third-party NormalizationRules may contribute additional component types to preserve
     * (denylisted container/block-entity/bundle components are stripped inside the API before
     * this returns them - see JourneyModeAPI.DENYLISTED_COMPONENTS).
     */
    public static ItemStack getNormalizedStack(ItemStack original) {
        ...
        for (net.minecraft.core.component.DataComponentType<?> type :
                com.aryangpt007.journeymode.api.JourneyModeAPI.collectAdditionalNormalizationComponents(original)) {
            copyComponent(original, normalized, type);
        }
        return normalized;
    }

    /** Generic helper so a wildcard-captured DataComponentType<?> can be copied type-safely. */
    ...
    public static String getItemKey(ItemStack stack, RegistryAccess registryAccess) {
        ...
        boolean hasSubtype = normalized.has(DataComponents.POTION_CONTENTS) ||
        ...
        if (!hasSubtype) {
            for (net.minecraft.core.component.DataComponentType<?> type :
                    com.aryangpt007.journeymode.api.JourneyModeAPI.collectAdditionalNormalizationComponents(stack)) {
    ...
    public void checkPendingUnlocks(RecipeManager recipeManager, RegistryAccess registryAccess) {
        ...
        for (Map.Entry<String, Integer> entry : new HashMap<>(collectedCounts).entrySet()) {
            ...
            ItemStack stack = itemStackFromKey(key, registryAccess);
    ...
    public void updateFromSync(Map<String, Integer> counts, Set<String> unlocked, Map<String, Long> timestamps) {
        ...
        // The very first sync after connecting (client attachment starts empty) must be silent,
     ...
     * 3-argument overload never carried `enabled`/`showTooltips`, so the client's own data
     * attachment instance (a separate object from the server's, even in singleplayer) silently
     * never reflected /journeymode off or a tooltip preference change - this overload is now the
     * one actually used by the packet handler.
     */
