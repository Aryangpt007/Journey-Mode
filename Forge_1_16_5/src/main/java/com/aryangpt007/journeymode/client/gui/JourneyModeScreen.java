...
public class JourneyModeScreen extends ContainerScreen<JourneyModeMenu> {
    ...
    protected void renderBg(MatrixStack matrixStack, float partialTick, int mouseX, int mouseY) {
        ...
        // Draw tabs
        drawTab(matrixStack, x + DEPOSIT_TAB_X, y + TAB_Y_OFFSET, "Deposit", currentTab == Tab.DEPOSIT);
        drawTab(matrixStack, x + JOURNEY_TAB_X, y + TAB_Y_OFFSET, "Journey", currentTab == Tab.JOURNEY);
        drawTab(matrixStack, x + STATS_TAB_X, y + TAB_Y_OFFSET, "Stats", currentTab == Tab.STATS);
        ...
        if (teamName != null) {
            this.font.draw(matrixStack, "Team: " + teamName, x + DEPOSIT_TAB_X, y + TEAM_BADGE_Y_OFFSET, 0xFFFFFF55);
        }
        ...
        if (currentTab == Tab.DEPOSIT) {
            ...
            renderStatsTab(matrixStack, x, y);
        }
    ...
    private void drawTab(MatrixStack matrixStack, int x, int y, String label, boolean selected) {
        ...
        fill(matrixStack, x, y, x + TAB_WIDTH, y + TAB_HEIGHT, bgColor);
    ...
    private void renderDepositTab(MatrixStack matrixStack, int x, int y) {
        ...
        if (hasItem) {
            ...
            if (!slotItem.isEmpty()) {
                ...
                if (alreadyUnlocked) {
                    this.font.draw(matrixStack, "§aAlready Unlocked!", x + 8, infoY, 0x00FF00);
                } else {
            ...
            this.font.draw(matrixStack, "Unlocked: " + data.getUnlockedCount() + " items", x + 8, infoY, 0x404040);
        }

        renderDepositAllButton(matrixStack, x, y);
    }
    ...
    private void renderDepositAllButton(MatrixStack matrixStack, int x, int y) {
        ...
        fill(matrixStack, buttonX, buttonY, buttonX + buttonWidth, buttonY + buttonHeight, border);
        fill(matrixStack, buttonX + 1, buttonY + 1, buttonX + buttonWidth - 1, buttonY + buttonHeight - 1, bg);
        ...
        this.font.draw(matrixStack, label, textX, buttonY + 4, 0xFFFFFFFF);
    }
     ...
     */
    private void renderStatsTab(MatrixStack matrixStack, int x, int y) {
        JourneyDataAttachment data = this.menu.getJourneyData();
        ...
        this.font.draw(matrixStack, "Unlocked: " + unlockedCount + " / " + totalResearchable, x + 8, y + 8, 0x404040);
        this.font.draw(matrixStack, "Complete: " + percent + "%", x + 8, y + 19, 0x404040);
        this.font.draw(matrixStack, "By mod:", x + 8, y + 32, 0x606060);
        ...
        for (java.util.Map.Entry<String, Integer> entry : perNamespaceTotal.entrySet()) {
            ...
            this.font.draw(matrixStack, label, x + 12, y + STATS_ROWS_TOP + drawn * STATS_ROW_HEIGHT, 0x404040);
            drawn++;
        }

        renderStatsScrollbar(matrixStack, x, y, perNamespaceTotal.size(), maxScroll);
        ...
        if (latestKey != null) {
            ...
            this.font.draw(matrixStack, fitToWidth("Latest: " + latestName, STATS_ROW_WIDTH), x + 8, y + STATS_LATEST_Y, 0x606060);
        }
    }
     ...
     */
    private void renderStatsScrollbar(MatrixStack matrixStack, int x, int y, int totalRows, int maxScroll) {
        ...
        fill(matrixStack, trackX, trackTop, trackX + STATS_SCROLLBAR_WIDTH, trackBottom, 0xFF373737);
        ...
        fill(matrixStack, trackX, thumbY, trackX + STATS_SCROLLBAR_WIDTH, thumbY + thumbHeight, 0xFFC6C6C6);
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        // TextFieldWidget.keyPressed only consumes control keys (backspace/arrows/select-all/etc.)
        // and returns false for plain character keys, since those are handled by charTyped
        // instead. Left unguarded, that lets a letter key bound to a vanilla keybind (e.g. "E"
        // for inventory-close) bubble past the field and close this screen mid-type. Swallow
        ...
        if (currentTab == Tab.JOURNEY && button == 0) { // Left click
            ...
            for (int i = startIndex; i < endIndex; i++) {
                ...
                if (mouseX >= itemX && mouseX < itemX + 16 && mouseY >= itemY && mouseY < itemY + 16) {
                    ...
                    ItemStack requested = JourneyDataAttachment.itemStackFromKey(itemId);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        if (currentTab == Tab.STATS) {
            ...
            statsScrollOffset = Math.max(0, Math.min(maxScroll, statsScrollOffset - (int) delta));
            return true;
        }
