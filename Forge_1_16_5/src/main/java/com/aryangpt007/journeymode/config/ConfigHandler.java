...
public class ConfigHandler {
    ...
    private static void initializeBlacklist() {
        if (!Files.exists(BLACKLIST_FILE)) {
            ...
            try {
                Files.write(BLACKLIST_FILE, GSON.toJson(defaultBlacklist).getBytes(StandardCharsets.UTF_8));
    ...
    private static void initializeThresholds() {
        if (!Files.exists(THRESHOLDS_FILE)) {
            ...
            try {
                Files.write(THRESHOLDS_FILE, GSON.toJson(defaultThresholds).getBytes(StandardCharsets.UTF_8));
    ...
    private static void loadBlacklist() {
        ...
        try {
            String content = new String(Files.readAllBytes(BLACKLIST_FILE), StandardCharsets.UTF_8);
    ...
    private static void loadThresholds() {
        ...
        try {
            String content = new String(Files.readAllBytes(THRESHOLDS_FILE), StandardCharsets.UTF_8);
            JsonObject json = new JsonParser().parse(content).getAsJsonObject();
