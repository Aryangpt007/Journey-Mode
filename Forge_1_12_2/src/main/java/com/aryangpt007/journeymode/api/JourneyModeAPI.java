...
public final class JourneyModeAPI {
    ...
    /** NBT keys that can never be whitelisted by a NormalizationRule, full stop. */
    public static final Set<String> DENYLISTED_KEYS;
    static {
        Set<String> denylist = new HashSet<>();
        denylist.add("BlockEntityTag");
        denylist.add("Items");
        denylist.add("Inventory");
        DENYLISTED_KEYS = Collections.unmodifiableSet(denylist);
    }
    ...
    public static Integer queryThresholdProviders(String itemKey) {
        for (ThresholdProvider provider : THRESHOLD_PROVIDERS) {
            try {
                ...
                LOGGER.error("ThresholdProvider " + provider.getClass().getName() + " threw while resolving " + itemKey, e);
            }
        }
        return null;
    }
    ...
    public static Set<String> collectAdditionalNormalizationKeys(NBTTagCompound originalTag) {
            ...
            try {
                ...
                for (String key : requested) {
                    if (DENYLISTED_KEYS.contains(key)) {
                        LOGGER.warn("NormalizationRule " + rule.getClass().getName() + " requested denylisted key '" + key + "' - ignored.");
                        continue;
                    }
                    keys.add(key);
                }
            } catch (Exception e) {
                LOGGER.error("NormalizationRule " + rule.getClass().getName() + " threw while resolving additional keys", e);
            }
        }
        return keys;
    }
