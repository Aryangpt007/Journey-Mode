 ...
 *
 * Client-only by design: ConfigHandler (a common/server-reachable class - /journeymode reloadconfig
 * can run on a dedicated server) never references this class directly. Instead ConfigHandler
 * exposes a generation counter (getRulesGeneration()) that this cache polls, so a dedicated
 * server with no client classes loaded never risks a NoClassDefFoundError.
 */
...
public final class CatalogStatsCache {
    ...
    private static void computeIfNeeded() {
        ...
        int total = 0;
        Map<String, Integer> perNamespace = new LinkedHashMap<String, Integer>();
    ...
    private static void computePlayerViewIfNeeded(IJourneyData data) {
    ...
    public static Map<String, Integer> getPerNamespaceUnlocked(IJourneyData data) {
    ...
    public static String getMostRecentlyUnlockedKey(IJourneyData data) {
        computePlayerViewIfNeeded(data);
