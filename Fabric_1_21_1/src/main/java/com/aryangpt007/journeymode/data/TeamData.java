 ...
 *
 * Data-Components port note: unlike the NBT-era reference (Forge_1_20_1), key generation and
 * stack reconstruction need a RegistryAccess (ItemStack.save()/parse() on 1.20.5+ operate on
 * Data Components and require the registry). Every method that touches the hybrid key therefore
 * takes a RegistryAccess parameter, mirroring JourneyDataAttachment's own method shapes on this
 * version rather than the Forge 1.20.1 signatures.
 */
public class TeamData {
    ...
    public boolean depositItem(ItemStack stack, RecipeManager recipeManager, RegistryAccess registryAccess) {
        ...
        String key = JourneyDataAttachment.getItemKey(stack, registryAccess);
    ...
    public boolean isUnlocked(ItemStack stack, RegistryAccess registryAccess) {
        return isUnlocked(JourneyDataAttachment.getItemKey(stack, registryAccess));
    }

    public int getCollectedCount(ItemStack stack, RegistryAccess registryAccess) {
        return collectedCounts.getOrDefault(JourneyDataAttachment.getItemKey(stack, registryAccess), 0);
    }
    ...
    public void checkPendingUnlocks(RecipeManager recipeManager, RegistryAccess registryAccess) {
        ...
        for (Map.Entry<String, Integer> entry : new HashMap<>(collectedCounts).entrySet()) {
            ...
            ItemStack stack = JourneyDataAttachment.itemStackFromKey(key, registryAccess);
    ...
    public JsonObject toJson() {
        ...
        members.forEach(uuid -> membersJson.add(uuid.toString()));
